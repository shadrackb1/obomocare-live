    ...
    <>
      <section className="relative w-full h-[70vh] min-h-[500px] flex items-end justify-center mb-section-gap -mt-20 overflow-hidden">
        <div className="absolute inset-0 z-0">
          <PlaceholderImage
            imgSrc={IMAGES.storiesHero}
            fallbackLabel="storiesHero"
            alt="Featured story hero image"
            className="w-full h-full object-cover"
            eager
          />
          <motion.div initial={{ scale: 1 }} animate={{ scale: 1.05 }} transition={{ duration: 10, repeat: Infinity, repeatType: "reverse", ease: "linear" }} className="absolute inset-0">
            <PlaceholderImage
              imgSrc={IMAGES.storiesHero}
              fallbackLabel="storiesHero"
              alt="Featured story hero image"
        ...
        <div className="relative z-10 w-full max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop pb-16 flex flex-col items-start text-on-primary pt-20">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="flex items-center gap-4 mb-stack-md">
          ...
          <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }} className="font-display text-4xl md:text-6xl font-bold mb-stack-md max-w-4xl text-on-primary drop-shadow-sm leading-tight">
            A Legacy of Care: The Matriarchs Rebuilding Communities.
          </motion.h1>
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.4 }} className="text-lg md:text-xl text-inverse-on-surface max-w-2xl mb-stack-lg opacity-90">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.6 }}>
            <Link to="/stories/1" className="inline-flex items-center gap-2 text-on-primary font-semibold text-sm border-b border-secondary-container pb-1 hover:text-secondary-container transition-colors group">Read the Full Story <ArrowRight className="group-hover:translate-x-1 transition-transform" size={18} /></Link>
          </motion.div>
        </div>
      </section>
          ...
          {stories.map((story, i) => (
            <motion.article initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} key={story.id} className="group cursor-pointer flex flex-col h-full bg-surface-container-lowest border border-outline-variant/30 hover:shadow-[0_0_30px_rgba(11,31,58,0.1)] transition-all duration-300 rounded-xl overflow-hidden">
              ...
              <div className="p-6 flex flex-col flex-grow">
                ...
                <h3 className="font-display text-2xl font-bold text-primary-container mb-3 group-hover:text-secondary-container transition-colors line-clamp-2">{story.title}</h3>
                <p className="text-on-surface-variant mb-6 line-clamp-3 flex-grow text-base">{story.excerpt}</p>
                <div className="mt-auto">
                  <Link to={`/stories/${story.id}`} className="inline-flex items-center gap-1 text-secondary-container text-sm font-semibold uppercase group-hover:translate-x-2 transition-transform">Read Article <ArrowRight size={16} /></Link>
                </div>
              </div>
            </motion.article>
          ))}
