    ...
    <>
      <header className="relative w-full min-h-[70vh] flex items-center justify-center overflow-hidden -mt-20">
        <div className="absolute inset-0 w-full h-full">
          <PlaceholderImage
            imgSrc={IMAGES.elderlySupport}
            fallbackLabel="elderlySupport"
            alt="Elderly Support"
          ...
          <motion.div animate={{ scale: [1, 1.2, 1], x: [-30, 0, -30], y: [-50, 0, -50] }} transition={{ duration: 15, repeat: Infinity, ease: "linear" }} className="absolute top-1/4 left-1/4 w-96 h-96 bg-secondary-container/30 rounded-full mix-blend-screen filter blur-[100px] opacity-60" />
        </div>
        <div className="relative z-10 max-w-container-max mx-auto px-4 md:px-12 w-full text-center md:text-left pt-20">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="max-w-3xl bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-8 md:p-12 shadow-[0_0_40px_rgba(11,31,58,0.2)]">
            ...
            <p className="text-lg text-inverse-on-surface mb-8 opacity-90 leading-relaxed">Providing vital home-based medical care, nutritional support, and social companionship to vulnerable elderly community members.</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start">
              <Link to="/get-involved" className="bg-secondary-container text-white font-bold py-4 px-8 rounded hover:shadow-[0_0_20px_rgba(253,118,26,0.4)] hover:-translate-y-1 transition-all text-center">Sponsor an Elder</Link>
              <Link to="/stories" className="border border-on-primary text-on-primary font-bold py-4 px-8 rounded hover:bg-white/10 transition-colors text-center">Read Their Stories</Link>
            </div>
          </motion.div>
        </div>
      </header>
      ...
      <section className="py-20 md:py-32 px-4 md:px-12 max-w-container-max mx-auto bg-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-primary-container p-8 rounded-xl h-full flex flex-col justify-center text-on-primary shadow-lg relative overflow-hidden group">
              ...
              <div className="relative z-10">
                ...
                <ul className="space-y-4 font-medium">
                  <li className="flex items-center gap-3 transform hover:translate-x-2 transition-transform"><span className="w-2 h-2 rounded-full bg-secondary-container shadow-[0_0_10px_rgba(253,118,26,0.8)]"></span>Routine Health Screenings</li>
                  <li className="flex items-center gap-3 transform hover:translate-x-2 transition-transform"><span className="w-2 h-2 rounded-full bg-secondary-container shadow-[0_0_10px_rgba(253,118,26,0.8)]"></span>Medication Delivery & Management</li>
                  <li className="flex items-center gap-3 transform hover:translate-x-2 transition-transform"><span className="w-2 h-2 rounded-full bg-secondary-container shadow-[0_0_10px_rgba(253,118,26,0.8)]"></span>Nutritional Supplements</li>
                </ul>
              </div>
            </div>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.4 }} className="md:col-span-2 rounded-xl overflow-hidden relative h-[300px] border border-outline-variant/30 group">
            <PlaceholderImage
              imgSrc={IMAGES.elderlySupportDetail}
              fallbackLabel="elderlySupportDetail"
              alt="Elderly Support"
